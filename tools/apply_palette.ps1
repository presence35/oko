...
foreach ($f in $files) {
    ...
    $text = $text.Replace('import com.odesaplay.oko.ThreatPalette', 'import com.odesaplay.oko.theme.AppPalette')
    $text = [regex]::Replace($text, '\bThreatPalette\b', { param($m) 'AppPalette' })

    # Import injection for consumers outside the theme package
    if ($text.Contains('AppPalette') -and -not $text.Contains('import com.odesaplay.oko.theme.AppPalette') -and $rel -notlike 'theme\*') {
        $text = [regex]::Replace($text, '(?m)^(package [^\r\n]+)\r?\n', { param($m)
            $m.Groups[1].Value + "`r`n" + 'import com.odesaplay.oko.theme.AppPalette' + "`r`n"
        }, 1)
    }
