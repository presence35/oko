...
- Map: threat icons now actually move — dead-reckoning was gated on `velocity.bearingDeg`, which the real NEPTUN feed never sends (only top-level `heading`), so every track held its raw fix; the engine now glides any active track with a resolvable course (bearing → heading → measured from the source's own fixes), anchored on the latest update / Мапа: іконки загроз тепер справді рухаються — dead-reckon був заблокований через `velocity.bearingDeg`, якого реальний фід NEPTUN не надсилає (тільки верхньорівневий `heading`), тож кожен трек лишався на сирій точці; рушій тепер рухає будь-який активний трек із відновлюваним курсом (bearing → heading → виміряний з власних фіксів джерела), якір — найсвіжіше оновлення
- Map: Just Fun toggle fixed — the `threat_icon_zoom` pref flowed at the wrong combine index, scrambling 8 settings (flyby/MiG, death anim, follow bullet, tally, group collapse, critical offline, Just Fun master); now aligned / Мапа: виправлено перемикач Just Fun — прапорець `threat_icon_zoom` йшов за хибним індексом combine і переплутував 8 налаштувань (flyby/МіГ, анімацію збиття, супровід кулі, лічильник, згортання груп, критичний офлайн, головний перемикач); тепер вирівняно
- Map: threat icons scale with zoom (1x→3x capped), tracked live to the zoom gesture; new Just Fun toggle "Zoom threat icons with map" turns it off for constant-size icons / Мапа: іконки загроз масштабуються з наближенням (1x→3x з обмеженням), слідкуючи за жестом збільшення; новий перемикач у «Просто розвага» — «Збільшувати іконки загроз з мапою» — вимикає це для постійного розміру
...
- Types: cruise missiles and FPV drones no longer misclassified as slow — vibration level, Settings/wizard grouping and typical-speed display now match the engine catalog / Типи: крилаті ракети та FPV-дрони більше не класифікуються як повільні — рівень вібрації, групування в налаштуваннях/майстерні та показ типової швидкості тепер відповідають каталогу рушія
- Map: threat taps respond instantly — the haptic tick and popup fire on finger-up instead of waiting out osmdroid's ~300 ms double-tap window / Мапа: дотики по загрозах миттєві — вібрація та картка спрацьовують на відпусканні пальця замість очікування ~300 мс вікна подвійного тапу
- Connection: tapping the status pill reopens the system-status dialog — per-source dot, three-tier legend, NEPTUN attribution link, and a Logs button / З'єднання: дотик до статус-пігулки знову відкриває діалог статусу системи — джерело, легенда трьох станів, посилання на NEPTUN та кнопка "Логи"
