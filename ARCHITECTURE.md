...
| `UkraineBorder.kt` | Static `UKRAINE_BORDER`: the outer hull of the oblast boundary polygons (`OblastBoundaries.kt`) — same source (EugeneBorshch/ukraine_geojson), same RDP simplification — generated by `tools/gen_ukraine_border.py`, so the country edge coincides with the combined red oblast fills. Rendered as a subtle transparent-fill `Polygon` ring in `MapView`. |
