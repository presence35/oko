...
| `engine/ThreatSource.kt` | `ThreatSource` plugin interface + `PluginConnectionState` (source-agnostic connection state) + `SourceType` (WS/REST) + `OperationalMode` (STREAMING/POLLING/STANDBY) + `setEnabled`. |
