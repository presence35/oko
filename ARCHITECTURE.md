...
| `WidgetSnapshot.kt` | `WidgetSnapshot` + pure `computeWidgetSnapshot(...)` — deterministic projection of threat state for the widget, computed via the engine (`ThreatEngine(NEPTUN_TYPES).evaluate`, `engine.isStale`, `distanceFlat`, `resolveFocus`); `officialAlert` comes from `eval.focusOblastAlertActive` and `officialYellowAlert` from `eval.focusOblastYellowAlertActive`. Counts + per-type `typeCounts` mirror the footer-strip semantics; `primaryThreat` = nearest live threat (id + position) so the widget can reveal it. Takes `degraded`/`offline` booleans derived by the caller from `PluginRegistry` (mirror rule) and maps them to the three-tier pill: `sourceOnline = !offline`, `sourceDegraded = degraded`. Takes `Map<String, NormalizedThreat>` + `List<OblastAlert>` directly (no `NeptunState`). Tested by `WidgetSnapshotTest`. |
