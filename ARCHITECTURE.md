...
| `LocationTracker.kt` | `object` singleton. Coarse `NETWORK_PROVIDER` only (~2 min / 250 m), falls back to last known -> `StateFlow<LatLng?>`; tracks the last fix time (`lastFixAtMs`) and offers a `forceRefresh()` GPS one-shot for the shelter screen (falls back to an active network one-shot when only coarse permission is granted). On start, while following with no fresh fix, kicks that GPS one-shot so the location dot arrives promptly (no waiting on the cell-tower listener). A 15-min periodic GPS sync loop runs when enabled (pref `periodic_gps_enabled`, default off) so Android sees real GPS access and cell drift is corrected. || `BatteryOptimization.kt` | Battery-exemption helpers. |
