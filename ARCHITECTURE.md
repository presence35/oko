...
| Official oblast alerts | `PluginRegistry.allAlerts` (priority-ordered sources, takeover merge; primary = `NeptunConnectionClient.alerts`; no REST fallback — `UbillingPlugin` removed 2026-09 for 1–2d stale sentinel data); **derivation** (`officialAlertActiveFor`, `redCities`, `raionName`, reason) owned by `engine/` | UI, `AlertService`, widget |
...
| `plugins/TestPlugin.kt` | Peace-time simulator (`sourceType = WS`). While enabled it fetches `testplugin.json` from the update server and plays a timed script of threat/alert events. Full-capability: random bursts aimed at the focus (any type, any distance range — equal min/max tolerated), a MiG-31K AVIATION takeoff (fixed or ranged distance) that auto-triggers the flyby, fake oblast/raion alerts, **explicit threats with region/district/locality/advisory/uncertainty/area-only/count/reliability/status passthrough**, **moving threats stepped along a `path`** (updates position/bearing/`updatedAtMillis` so dead-reckoning and zone enter/exit run for real), **`resolve`/`clearThreats` removal events**, and clears. A failing script event is caught and logged (`lastError`) instead of killing the app; movers are cancelled on resolve/clear/disable. Reports CONNECTED while running; `stop()`/disable clears its output — exactly like a real source. `typeCatalog` empty (inherits NEPTUN_TYPES via registry merge). Disabled by default. |
| `plugins/PluginRegistry.kt` | Health authority: manages plugins; **every merge/health derivation reads `enabledPlugins` (registered sources the user switched on), never the raw registration list** — so a disabled source can't feed, own, or degrade anything. Merges threats/alerts with **takeover** semantics (authoritative source's snapshots are sole truth; stale holders fill only when nothing is authoritative), exposes `perSourceState`, `wsHealthy` (an enabled WS source is `CONNECTED` — a disabled/silent/down WS is not delivering), `degraded` (`!wsHealthy` — the orange feed-freshness tier, immediate; a switched-off source counts as not delivering, so it degrades), `degradedSince` (episode start for the offline escalation, stamped on the **monotonic** clock), `coveredByFallback` (an enabled non-WS source delivering real data: `POLLING` + `CONNECTED`), `isOffline(now)` (red tier = degraded past `OFFLINE_EPISODE_MS` with no fallback delivering; `now` is monotonic), `lastThreatUpdateAt` + `isThreatDataStale(now)` (**source-agnostic** staleness: stale only when NO source delivered a threat update within `THREAT_DATA_STALE_MS` — a live source clears it, so its threats are never gated behind another source's quiet feed), `activeAlertSource`, aggregate `connectionState` (worst over enabled sources), merged `Threat` feed + `typeCatalog`, and a `sourceEvents` SharedFlow (toggles + alert-owner handovers) feeding the live connection log / Sources-tab activity. For alerts only a **CONNECTED** WS socket is authoritative — `DEGRADED` (quiet >30s) is stale data and falls back to the union-hold instead of owning the feed. REST sources activate off `wsHealthy`, never by observing a sibling plugin. Threats merge by concatenation; alerts by their own key, so a Test-source fake alert only owns oblasts no live source covers. |
| `AppPluginHolder.kt` | Singleton holding `PluginRegistry`; registers Neptun (WS) then Test (WS, disabled by default); exposes `appForeground` (retained for future REST sources) + `setAppForeground`. Initialized in `AlertService.onCreate` and `MainViewModel.init`. Ubilling REST fallback removed 2026-09 (stale `1970-01-01` sentinel, 1–2d delay vs NEPTUN WS). |
...
| `WidgetSnapshot.kt` | `WidgetSnapshot` + pure `computeWidgetSnapshot(...)` — deterministic projection of threat state for the widget, computed via the engine (`ThreatEngine(NEPTUN_TYPES).evaluate`, `engine.isStale`, `distanceFlat`, `resolveFocus`); `officialAlert` comes from `eval.focusOblastAlertActive`. Counts + per-type `typeCounts` mirror the footer-strip semantics; `primaryThreat` = nearest live threat (id + position) so the widget can reveal it. Takes `degraded`/`offline` booleans derived by the caller from `PluginRegistry` (mirror rule) and maps them to the three-tier pill: `sourceOnline = !offline`, `sourceDegraded = degraded`. Takes `Map<String, NormalizedThreat>` + `List<OblastAlert>` directly (no `NeptunState`). Tested by `WidgetSnapshotTest`. |
...
| `LocationTracker.kt` | `object` singleton. Coarse `NETWORK_PROVIDER` only (~2 min / 250 m), falls back to last known -> `StateFlow<LatLng?>`; tracks the last fix time (`lastFixAtMs`) and offers a `forceRefresh()` precise fix for shelters / calibration — it retries GPS a few times (~24s) before falling back to an active network one-shot, and works with coarse-only permission. On start with no fresh fix it seeds a fast cell-tower one-shot and, while following, kicks `forceRefresh()` so the dot arrives promptly. A 15-min periodic GPS sync loop runs when enabled (pref `periodic_gps_enabled`, default off) so Android sees real GPS access and cell drift is corrected. || `BatteryOptimization.kt` | Battery-exemption helpers. |
...
- **Official alerts come from a priority-ordered set of sources, one live owner at a time.**
  ...
  **Connection health is three-tier, and the tiers mean different things.** Green = a WS source is
  ...
  outranks degraded in the pill. The degraded/offline derivation lives once in `PluginRegistry`;
