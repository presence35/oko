...
| `Threat.kt` | NEPTUN display metadata + parsing: `ThreatType`/`ThreatTypeCatalog`/`Reliability` (labels, staleness, nominal speeds), `OblastAlert`/`inOblast` (prefix or whole-word match, so Crimea's republic form hits the "Крим" stem)/`isOblastWide`/`coversCity` (oblast-wide alerts cover the whole stem) + shared `officialAlertActiveFor` scope gate; `translateCourseAssessment` (EN course text, word-level common-word translation); `normalizedThreatFromJson` — NEPTUN JSON → `NormalizedThreat` directly (the engine currency; no `Threat` display DTO anymore). |
