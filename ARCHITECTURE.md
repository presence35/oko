...
| `domain/ThreatBehavior.kt` | Threat motion + inbound staging (moved from `ui/`). `BehaviorOutcome`/`ThreatBehavior`/`resolveThreatBehavior` glide markers; `isInbound` (source-named destination inside the yellow ring) + `stageInbound` (re-tags one engine result: slow inbound → OUTER, fast inbound → INNER) + `approachRingKm`/`stagedTier`; `OrbitBehavior` rides an inbound track on the yellow ring around its destination, and only when its raw fix is already within the red gate, instead of parking it on the city centre. Applied by `MainViewModel` and `AlertService`; the engine stays a pure reporter. Widget not staged yet (ROADMAP). |
