...
| `WidgetSnapshot.kt` | `WidgetSnapshot` + pure `computeWidgetSnapshot(...)` — deterministic projection of threat state for the widget, computed via the engine (`ThreatEngine(NEPTUN_TYPES).evaluate`, `engine.isStale`, `distanceFlat`, `resolveFocus`); `officialAlert` comes from `eval.focusOblastAlertActive`. Counts + per-type `typeCounts` mirror the footer-strip semantics; `primaryThreat` = nearest live threat (id + position) so the widget can reveal it; `sourceOnline` is grace-filtered like the app pill; `sourceDegraded` is `true` when the socket is quiet OR a fallback is covering (`coveredByFallback`). Takes `ConnectionState` + `Map<String, NormalizedThreat>` + `List<OblastAlert>` directly (no `NeptunState`). Tested by `WidgetSnapshotTest`. |
...
- **Official alerts come from a priority-ordered set of sources, one live owner at a time.**
  ...
  silent/`DEGRADED`), and `coveredByFallback` (no WS delivering but a non-WS source authoritative).
  The merged alert feed uses **takeover** semantics, not union: while a source is *authoritative*
  (WS socket CONNECTED, or REST poller engaged and having actually fetched), its snapshots are the
  ...
  **Connection health is three-tier, not two.** Green online / red offline stay grace-filtered
  as before; the orange **degraded** middle state means the primary live feed is *not delivering
  normally* while coverage still exists. Two flavors produce it: the raw socket is connected but
  no frame has arrived for `DEGRADED_STALE_MS` (30 s, below the REST refresh and watchdog
  thresholds) — "threats are delaying" — or the primary WS source is down/silent/disabled while a
  fallback source is actively covering (`coveredByFallback`, e.g. NEPTUN off + Ubilling polling).
  It is advisory only: it never suppresses tiering, alerts, or the widget, and offline always
  outranks it in the pill unless a fallback is covering (then degraded wins — coverage exists).
