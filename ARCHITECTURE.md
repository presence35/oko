...
| File | Responsibility |
| --- | --- |
| `connection/ConnectionState.kt` | Sealed interface state machine: `Disconnected` → `Connecting` → `Connected` → `Degraded` → `Offline`; `ConnectionMilestone` (M3/M5_CRITICAL/M6/M10/M20_GAVE_UP, once per episode); convenience extensions `isConnected`, `isDegraded`, `isOffline`, `offlineSinceOrNull`, `reconnectStartMillisOrZero`. Transport-internal (not the consumer SPI). |
