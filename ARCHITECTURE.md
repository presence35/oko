...
- **Position prediction** (both consumers): the full contract — `SpeedCache` speed priority,
  ...
  consumers use, so an approximate track heading to your area patrols its ring (slow yellow,
  fast red) instead of parking on the position.
...
| File | Responsibility |
| --- | --- |
| `MainActivity.kt` | Single activity; dark theme (via `DarkThemePlugin`); starts `AlertService`; legacy osmdroid cache cleanup. *Note:* location→notification permissions defer until the wizard resolves (the contextual battery prompt no longer gates them); "Later" sets `permission_prompt_deferred` (re-armed each cold start). |
...
| `ConnectionLog.kt` | `object` singleton. Persisted ring buffer (last 50 episodes) fed by the watchdog; sub-15s flaps never commit (production grace), longer drops bracket with a recovery row. `ConnStatus` = ONLINE/OFFLINE/DEGRADED — every drop records an `OFFLINE` episode. `ConnLogEntry.activeSource` records which source owned the alert feed during the episode (null = primary). Pure `commitLogState` (tested); rendered in the Logs screen. Non-blocking persistence (no `runBlocking` on IO). |
...
| `engine/OblastUtils.kt` | Geographic/text utilities (kept from the old `ThreatEvaluator`): `inOblast`, `inFocusOblast`, `matchOblast`, `resolveOblastId`, `threatBody`, `alertRegionName`, `coversCityRaion` (scoped raion coverage matching for cities), `OblastMatch`. *Note:* `alertRegionName` renders the alert's region in the UI language (UA raw / EN transliterated / RU real Russian form) for the official-reason fallbacks; the nearest-in-zone reason picker lives on `ThreatEngine.deriveOfficialAlertReason`. |

### Domain logic

| File | Responsibility |
| --- | --- |
| `NightMode.kt` | Shared night helpers for **both** consumers (mirror rule): `isNightActive`, `effectiveZoneParams`/`effectiveArmed`, `effectiveOfficialEnabled`, `NightConfig`/`NightZones`/`ZoneArmed`, and `NightSleepPreset` (the encoded snapshot behind "Just let me sleep!"). |
| `domain/ThreatBehavior.kt` | Threat motion + inbound staging (moved from `ui/`). `BehaviorOutcome`/`ThreatBehavior`/`resolveThreatBehavior` glide markers; `isInbound` (source-data predicate) + `stageInbound` (re-tags one engine result: slow inbound → OUTER, fast inbound → INNER) + `approachRingKm`/`stagedTier`; `OrbitBehavior` places an inbound track on its ring around its destination (else the focus) instead of parking it on the city centre. Applied by `MainViewModel` and `AlertService`; the engine stays a pure reporter. Widget not staged yet (ROADMAP). |
| `Cities.kt` | ~483 places grouped by oblast in three zoom tiers (`CityTier`: 26 curated MAJOR always / MEDIUM from mid-zoom — 14 curated non-seats only / rest MINOR up close; non-curated places derived from GeoNames CC BY 4.0 via `tools/gen_cities.ps1`, 2 km dedupe, same-name towns resolve to the major city first, then by population) + `CityLabelOverlay` (colors labels red for the `redCities` set — region-precise: whole oblast for a whole-oblast alert, covered cities only for a raion alert; a red-filled polygon keeps its labels white; MAJOR labels reveal progressively by `MajorReveal` — top-5 OVERVIEW from the country view, 8 MID from mid-zoom, the rest LATE up close); EN names from the app's own КМУ №55 transliteration; `resolveFocus` maps focus point → oblast stem via canonical `cityOblastId` (majors only). |
...
| `domain/RussianToponyms.kt` | Display-only Ukrainian-to-Russian toponym forms for city, oblast and raion names when the app language is RU. Explicit city/oblast tables plus a raion suffix rule and overrides; unknown names fall back to the Ukrainian original. Never used for matching (identity is canonical-ID). |
| `community/CompactOblastBoundaries.kt` | Compact oblast boundary polygons (scaled-int rings, `canonicalId`/`get`/`contains`). Replaces the old `OblastBoundaries.kt`. Used by the engine (`computeFillKeys`, `deriveOfficialAlertReason`) and map layers for region-precise fills. |
| `community/CompactRaionBoundaries.kt` | Compact post-2020 raion boundary polygons (`forKey(oblastStem, raionName)`, scoped by parent oblast; unknown names return null, never the parent oblast). Replaces the old `RaionBoundaries.kt`. Keys match `CityRaions` and NEPTUN's raion alert names. |
| `community/CompactPolygon.kt` | Scaled-int ring geometry (`ScaledRing`, `LatLon`): sub-70m boundary precision, point-in-polygon. Shared by both compact boundary tables. |
...
| `UserPrefs.kt` | (formerly `ZonePrefs.kt`) `ThreatCardSize`/`ThreatIconSet`/`OverlapMode` + DataStore store (`user_prefs`): all toggles/thresholds/follow/pin/visibility, night config, onboarding flag (`wizard_completed`), `boot_restart_enabled`.
| `domain/UserPreferences.kt` | `UserPreferences` data class + `AppLanguage`/`ThreatCardSize`/`ThreatIconSet`/`OverlapMode`/`AlertRegionMode`/`MoraleVoice` enums: the schema `UserPrefs` persists and `MainViewModel`/`WidgetUpdater` consume. | Language is NOT stored: `systemLanguage()` maps the phone locale (`ru`→RU, `uk`→UA, else EN) fresh on every startup. `haptics_enabled` is tri-state (absent = follow the system haptic setting). Also `threatMapFlow`/`threatAlertFlow`; the resolved-threat tally's focus-oblast default + "All of Ukraine" opt-in (`neutralized_tally_all_ukraine`). ~74 preference keys — wide, but genuinely just user-facing preferences now; the persisted *operational* state that used to live here (`ConnectionLog`, offline-restore, official-announced identity) has moved to `ServiceState`. |
| `Strings.kt` | UA/EN/RU `StringSet` tables in `lang/` (never Android resource localization); content selection goes only through `AppLanguage.pick()` + per-domain accessors (`ThreatTypeInfo.label()`, `City.name()`, `FocusAttribution.bannerCity()`, …) — never `==` on the language; `formatRelativeTime`, `formatDateTime` (app language, not device locale). RU tables ship real RU translations. |
...
| `UkraineBorder.kt` | Two static outlines generated by `tools/gen_ukraine_border.py`: `UKRAINE_BORDER` — the closed outer hull of the oblast boundary polygons (`community/CompactOblastBoundaries.kt`), and `UKRAINE_LAND_BORDER` — the same ring with coastline edges removed, rendered as a GPU GeoJSON line layer in MapLibre. |
| `SettingsScreen.kt` | Collapsible sections: map centre, per-type Map/Alerts toggles + icon packs + icon zoom, card size, alert toggles, the **Morale** section (last card: calm messages, shoot-down animation + follow-the-bullet, neutralized count + all-Ukraine — calm messages is effective only when the Morale master is on), night-mode card, updates, battery exemption, guide; one-time explainers. The night-mode section tints its whole collapsible card darker purple + border (`NightSectionBg`/`NightSectionBorder`), no moon icon on the enabled toggle; battery exemption lives in the Alerts section; shelter button toggle + shelter directory row live in their own dedicated **Shelter** section; sections stay user-collapsible even while searching; a Reset-tips row re-arms every first-use hint (toast counters + explainers); icon packs ship PHOTO/ARMY/COMIC/RUSSIAN only (CLASSIC removed — a stored `"CLASSIC"` pref falls back to PHOTO, classic vectors remain the internal pack-fallback); the card-size tiles preview the small card as its real compact top-left chip (~75% of tile width) and the large card full-width. |
...
| `FirstLaunchWizard.kt` | The 5-page first-run wizard (tips → threat care → location follow-me/city-pick → zone controls → core features), extracted from MainScreen; thin yellow→blue progress bar on the **top** edge of the Next pill, whole pill dims while disabled. *Note:* threat care is a 2-column vertical grid (fast ⚡ | slow 🐢, 52dp icons) with no icon-pack picker (icon packs live only in the Morale panel); the zone-controls step shows big red/yellow concentric circles, a REAL slow-red radius slider + armed toggle (wired to the same ViewModel setter as the map) with the slider hint beneath it, then vertical rows for shelter / red-dot / gear with short captions (`wizardShelterDesc`, `zoneRedLabel`, `editZonesLabel`); Morale is **on by default** on fresh installs (the master switch is the global flourish kill-switch, enforced by the engine gates, not just a panel toggle) with the "⚙ settings" mention rendered as an inline colored gear; every interactive control ticks via `pressTick`. Wizard exit fires a one-shot fake SHAHED shootdown near the focus (`WelcomeShootdown` → `DeathFxController.strike` with synthetic id `welcome-demo`, never touching threats/registry/tally), once per install (`welcome_shootdown_played`) and gated only on the Morale master. |
...
| `AlertWatchdog.kt` | WorkManager periodic worker (15 min) that restarts `AlertService` (background-safe) if `MonitoringStatus.running` is false and `bootRestartEnabled` is true; a blocked start posts a tap-to-resume prompt. Scheduled from `BootReceiver` and `MainActivity.onCreate`. Closes the process-kill silent dead state. |
| `BootReceiver.kt` | Restarts `AlertService` on `BOOT_COMPLETED` / `MY_PACKAGE_REPLACED`, gated on the user's `boot_restart_enabled` pref (Settings → Alerts). When monitoring is not auto-restarted, posts a persistent "Monitoring paused after reboot — tap to start" notification. Always schedules `AlertWatchdog`; a blocked restart posts a tap-to-resume prompt. |
...
- **Onboarding gates on `wizard_completed`.** The dedicated flag is what
  ...
  re-runs via "Replay first launch" (which clears only this flag). There is no language
  choice: the app language is the phone locale (`ru`→RU, `uk`→UA, else EN), resolved fresh
  on every startup with no stored override.
...
- **Core gate: plugin work never touches core files.** Gated: `engine/**` (evaluation,
  tiers, band, staleness, oblast matching), `service/AlertService.kt` (lifecycle +
  reconcile skeleton), `service/DebugLog.kt` (audit schema + wire format). Everything
  else — plugin, settings UI, strings, map/flourish — is un-gated. An exception
  ("unless required", e.g. threading `prevTiers` through `evaluate`) is allowed only
  with all three in the same change: (1) justification recorded (commit message says
  what and why), (2) `BEHAVIORS.md`/`ARCHITECTURE.md` updated alongside, (3) unit
  tests covering the core change. No exceptions to (2): undocumented core changes
  are how the phantom "20s grace" happened.
...
- **Night mode is shared, not mirrored.** Both sides call `isNightActive`/
  ...
  **"Just let me sleep!"** is a settings preset, not a service gate: the button writes the
  existing night prefs (four bells off, both siren overrides off, both night official enables
  off, custom zones on so `effectiveArmed` actually applies the bells), remembering the prior
  values in `NightSleepPreset` (stored as `night_sleep_restore`) so the next tap restores them.
  The service only resolves the two new night official enables with the existing
  `if (nightActive)` idiom via `effectiveOfficialEnabled` — exactly like the night siren
  overrides — and resolves zone arming through the same `effectiveArmed` the ViewModel uses.
  The button only exists inside an enabled night card.
...
- **Focus point — one shared resolver.** `followMe` → camera + zones + alerts centre on the
  ...
  `resolveFocus` → `cityOblastId` canonical match, **major cities only** — the ~300 minors are
...
preferences: toggles, thresholds, follow/pin, night config, icon packs, and the
...
settings surface (`SettingsScreen.kt`'s per-type toggles / icon packs / card size /
Morale / night-mode / updates / battery / shelter sections) and no longer mixes concerns —
